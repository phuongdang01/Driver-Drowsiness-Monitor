...
class SignalFeaturePipeline:
    ...
    def __init__(self, config: RuntimeConfig):
        ...
        # DynamicMAR is used exactly in the style of the user's MAR analysis files
        # (evaluate_mar_videos.py): window_size=150, factor=0.7, floor=0.15.
        # Do not replace it with the personalized baseline/MAD/min_gap variant.
        self.dynamic_mar = DynamicMAR(
            window_size=int(getattr(config.thresholds, "mar_window", 150)),
            factor=float(getattr(config.thresholds, "mar_factor", 0.7)),
            floor=float(getattr(config.thresholds, "mar_min", 0.15)),
        )
